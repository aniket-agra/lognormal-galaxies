...
int main(int argc, char* argv[]){
	...
	Nijk = new double [nrtot*num_interlacing];
	#pragma omp parallel for
	for (int i = 0; i<nrtot*num_interlacing;i++) Nijk[i] = 0;
	...
	for (int i=0;i<Ngal;i++){
		...
		for (int j=0; j<num_interlacing; j++){
			...
			if(flag_mass_assign == 0){
				distribute_ngp(galtmp, Nijk, j);
			}else if(flag_mass_assign == 1){
				distribute_cic(galtmp, Nijk, j);
			}
		}
	}
	delete[] galtmp; 
	...
	fftw_complex *deltak[num_interlacing];
	double *deltar[num_interlacing];
	fftw_plan plan_delta[num_interlacing];
	for (int i=0; i<num_interlacing; i++){
		deltak[i] = (fftw_complex*) fftw_malloc(n0_tmp*n1_tmp*cn2_tmp * sizeof(fftw_complex));
		deltar[i] = (double *) deltak[i];
		plan_delta[i] = fftw_plan_dft_r2c_3d(n0_tmp,n1_tmp,n2_tmp,deltar[i],deltak[i],FFTW_ESTIMATE);
	}
	...
	for (int i0=0;i0<n0;i0++){
		for (int i1=0;i1<n1;i1++){
			for (int i2=0;i2<n2;i2++){
				...
				for (int i=0; i<num_interlacing; i++){
					deltar[i][indx2] = Nijk[indx+i*nrtot]/Nbar-1.;
				}
			}
		}
	}
				...
				if(real_dof){
					...
					if(flag_mass_assign == 0){
						...
						if(num_interlacing == 1){
							//W_fac = 1.0;
							Pk_add = (pow(deltak[0][ijk][0],2)+pow(deltak[0][ijk][1],2))/pow(W_fac,2);
	...
	for (int i=0; i<num_interlacing; i++) fftw_free(deltak[i]);
...
void ReadParams(int argc, char* argv[]){
	if (argc == 1){
		...
		if (check_int(argv[13],"flag_mass_assign")) flag_mass_assign = atoi(argv[13]);
		if (check_int(argv[14],"num_interlacing")) num_interlacing = atoi(argv[14]);
	}else{
		cout << "number of arguments should be 0 or 14!!!" << endl;
		exit(1);
	}
